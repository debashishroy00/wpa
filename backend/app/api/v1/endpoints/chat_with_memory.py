...
async def chat_with_intelligence(
    ...
    try:
        ...
        # Apply tax optimization intelligence if applicable (same as /message endpoint)
        logger.info(f"Checking tax query for message: '{request.message[:50]}...'")
        tax_keywords = ['tax', 'deduction', 'irs', 'itemize', 'roth', '401k', 'ira', 'bracket', 'savings', 'retirement contribution']
        if any(keyword in request.message.lower() for keyword in tax_keywords):
            try:
...
"""
                        ...
                        logger.info(f"Applied tax optimization intelligence for user {current_user.id} - Found ${tax_insights.get('total_potential_savings', 0):,.0f} in potential savings")
